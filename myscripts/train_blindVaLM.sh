...
CKPT_DIR=./CHECKPOINTS/checkpoint_blindVaLM_40600upd
...
srun torchrun --nproc_per_node=4 fairseq/fairseq_cli/train.py \
    ...
    --sample-break-mode none --max-tokens 65536 --max-update 40600 --save-interval-updates 5000 \
    --disable-validation --use-knn-datastore --use-joint-attention --joint-layer-index 2 \
    --ddp-backend legacy_ddp --wandb-project VaLM-baseline --fp16
