...
class CLIP_Text_Encoder:
    def __init__(self, new_dim, model_type="RN50x16"):#RN50x64

        self.device = "cuda" if torch.cuda.is_available() else "cpu"
        self.model, self.preprocess = clip.load(model_type, device=self.device)
        self.decoder = SimpleTokenizer()
        #print(sum(p.numel() for p in self.model.transformer.parameters() if p.requires_grad))
        #self.linear = nn.Linear(self.model.transformer.width, new_dim).to(self.device)
        #self.linear = self.linear.to(self.model.token_embedding.weight.dtype)  # Match the dtype
    ...
    def encode_text(self, prev_output_tokens, context_length=77):
        ...
        for sample in input_tokens:
            ...
            length_of_inner_lists = [len(sublist) for sublist in sub_sent_feature_list]

            whole_sent_feature = torch.cat(sub_sent_feature_list, dim=0)

            assert whole_sent_feature.shape[0] == length
            batch_features.append(whole_sent_feature)#sub_sent_feature_list)#whole_sent_feature)

        length_of_inner_lists = [len(sublist) for sublist in batch_features]
    ...
    def get_text_encoding(self, text):
        ...
        # Ensure the linear layer and input tensor have the same dtype
