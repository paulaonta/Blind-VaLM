...
def distributed_init(cfg: FairseqConfig):
    ...
    if not cfg.common.tpu:
        torch.cuda.set_device(cfg.distributed_training.distributed_rank)
...
def all_gather_list(data, group=None, max_size=16384):
    ...
    if (
        ...
        all_gather_list._buffer = torch.cuda.ByteTensor(buffer_size)#torch.tensor(buffer_size, dtype=torch.uint8, device='cuda')#torch.cuda.ByteTensor(buffer_size)
