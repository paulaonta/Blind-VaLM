...
class Trainer(object):
    ...
    def __init__(self, cfg: FairseqConfig, task, model, criterion, quantizer=None):
        ...
        if self.cuda and self.data_parallel_world_size > 1:
            ...
            self._grad_norm_buf = torch.tensor(self.data_parallel_world_size, dtype=torch.float64, device='cuda')
        else:
    ...
    def _check_grad_norms(self, grad_norm):
        ...
        if self._grad_norm_buf is not None:
            ...
            self._grad_norm_buf = grad_norm #[self.data_parallel_rank] = grad_norm
            distributed_utils.all_reduce(
                self._grad_norm_buf, group=self.data_parallel_process_group
            )

            def is_consistent(tensor):
                max_abs_diff = torch.max(torch.abs(tensor - tensor.item()))#tensor[0]))
                return (
                    (
                        torch.isfinite(tensor).all()
                        and (max_abs_diff / (tensor.item() + 1e-6) < 1e-6).all() #tensor[0]
                    )
